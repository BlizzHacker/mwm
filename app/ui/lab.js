...
const LAB = { path: "", result: null, status: null, quarantine: [] };
...
VIEWS.lab = async function () {
  ...
  const [status, quarantine] = await Promise.all([
    guard(() => invoke("arkana_status")), guard(() => invoke("lab_quarantine_list")),
  ]);
  if (current !== "lab") return;
  LAB.status = status || null;
...
function drawLabResult() {
  ...
  $("#lab-deep").onclick = async () => { const id = await guard(() => invoke("arkana_analyze", { path: LAB.path })); if (id) watchJob(id); };
  $("#lab-q").onclick = async () => {
