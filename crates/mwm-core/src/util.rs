...
/// Where MWM keeps registry backups and logs.
pub fn data_dir() -> PathBuf {
    ...
    let dir = if cfg!(windows) { base.join("MoveWeight").join("MWM") } else { base.join("MWM") };
    // One-time carry-over from the pre-rename (MDW) data folder.
    let old = if cfg!(windows) { base.join("MoveWeight").join("MDW") } else { base.join("MDW") };
