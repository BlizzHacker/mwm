...
fn copy_file(job: &Job, from: &Path, to: &Path) -> anyhow::Result<()> {
    ...
    let tmp = to.with_extension(format!("{}mwmpart", to.extension().map(|e| format!("{}.", e.to_string_lossy())).unwrap_or_default()));
    {
...
pub fn multi_rename(plan: &[(String, String)]) -> anyhow::Result<usize> {
    ...
    for (i, (old, new)) in plan.iter().enumerate() {
        let p = Path::new(old);
        let tmp = p.with_file_name(format!(".mwm-rename-{i}-{}", util::now_secs()));
...
mod tests {
    ...
    fn tmp(name: &str) -> PathBuf {
        let d = std::env::temp_dir().join(format!("mwm-files-{name}-{}", std::process::id()));
