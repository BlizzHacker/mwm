...
fn shred_file(p: &Path, passes: u32) -> std::io::Result<u64> {
    ...
    let mut nb = [0u8; 8];
    rng.fill(&mut nb);
    let tmp: PathBuf = parent.join(format!("{:016x}.mdw", u64::from_le_bytes(nb)));
...
pub fn wipe_free_space(dir: &str) -> anyhow::Result<String> {
    ...
    #[cfg(not(windows))]
    {
        let file = Path::new(dir).join(".mdw-wipe.tmp");
...
mod tests {
    ...
    fn shreds_and_removes() {
        let dir = std::env::temp_dir().join(format!("mdw-shred-{}", std::process::id()));
