...
pub const TASKS: &[Task] = &[
    ...
    t("wureset", "Windows repair", "Reset Windows Update", "Stops update services, sets the download cache aside, restarts them. Fixes stuck updates.", "net stop wuauserv /y & net stop bits /y & ren \"%windir%\\SoftwareDistribution\" SoftwareDistribution.mdw-%RANDOM% & net start bits & net start wuauserv", true, "1", ""),
    ...
    t("restore", "Windows repair", "Create restore point", "Snapshot of system settings and drivers - make one before big changes.", "powershell -NoProfile -Command \"Checkpoint-Computer -Description 'MDW restore point' -RestorePointType MODIFY_SETTINGS; 'Restore point created.'\"", true, "1-3", ""),
    ...
    t("battery", "Hardware", "Battery health report", "Design vs. full-charge capacity and usage history (opens in your browser).", "powercfg /batteryreport /output \"%LOCALAPPDATA%\\MoveWeight\\MDW\\battery-report.html\" && start \"\" \"%LOCALAPPDATA%\\MoveWeight\\MDW\\battery-report.html\"", false, "<1", ""),
    t("energy", "Hardware", "Power efficiency report", "60-second trace of what drains power / blocks sleep.", "powercfg /energy /output \"%LOCALAPPDATA%\\MoveWeight\\MDW\\energy-report.html\" /duration 60 & start \"\" \"%LOCALAPPDATA%\\MoveWeight\\MDW\\energy-report.html\"", true, "1", ""),
